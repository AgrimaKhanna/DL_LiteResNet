...
if __name__ == '__main__': 
    ...
    parser.add_argument('--resnet_architecture', default='LiteResNet', type=str, help='name of resnet architecture from config') 
    ...
    trainset = torchvision.datasets.CIFAR10(root='./data_10', train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=config["batch_size"], shuffle=True, num_workers=config["num_workers"])
    testset = torchvision.datasets.CIFAR10(root='./data_10', train=False, download=True, transform=transform_test)
    ...
    net, total_params = ResNetLite(config=config) 
